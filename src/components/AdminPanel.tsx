    ...
    return (
        <div className="min-h-screen bg-slate-50 text-slate-800 p-8 relative overflow-hidden font-sans">
            ...
            <div className="absolute top-0 left-0 w-full h-full overflow-hidden pointer-events-none z-0">
                <div className="absolute top-[-10%] left-[-10%] w-[40%] h-[40%] bg-violet-100 rounded-full blur-[100px] opacity-50"></div>
                <div className="absolute bottom-[-10%] right-[-10%] w-[40%] h-[40%] bg-fuchsia-100 rounded-full blur-[100px] opacity-50 delay-1000"></div>
            </div>
            ...
            <div className="max-w-6xl mx-auto relative z-10">
                <header className="flex justify-between items-center mb-8 border-b border-slate-200 pb-6 bg-white/80 backdrop-blur-md rounded-2xl px-8 py-4 shadow-sm">
                    <div>
                        <h1 className="text-3xl font-bold text-slate-900 tracking-tight">Admin <span className="text-violet-600">Dashboard</span></h1>
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
                    <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 group hover:border-violet-300 transition-colors">
                        <h3 className="text-violet-600 text-xs font-bold uppercase tracking-widest mb-2">Total Users</h3>
                        <p className="text-4xl font-light text-slate-900 group-hover:scale-105 transition-transform origin-left">{users.length}</p>
                    </div>
                    <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 group hover:border-fuchsia-300 transition-colors">
                        <h3 className="text-fuchsia-600 text-xs font-bold uppercase tracking-widest mb-2">Active Connections</h3>
                    ...
                    <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 col-span-2 flex flex-col justify-center gap-4">
                        <div className="flex gap-3">
                            <input 
                                ...
                                className="bg-slate-50 border border-slate-200 text-slate-900 px-4 py-3 rounded-xl flex-1 focus:outline-none focus:border-violet-500 focus:bg-white transition-all placeholder-slate-400"
                            />
                            <input 
                                type="text" 
                        ...
                        <button
                            ...
                            disabled={generating}
                            className="w-full flex items-center justify-center gap-2 bg-violet-50 hover:bg-violet-100 text-violet-700 border border-violet-200 font-bold py-3 px-6 rounded-xl shadow-sm hover:shadow-md transition-all active:scale-[0.98] disabled:opacity-50 disabled:cursor-not-allowed"
                        >
                            <Plus size={20} />
                ...
                <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
                    <div className="p-6 bg-slate-50 border-b border-slate-200 flex justify-between items-center">
                        ...
                        <button onClick={fetchUsers} className="p-2 hover:bg-gray-200 rounded-full transition text-violet-600 hover:shadow-sm">
                                ...
                                {users.length === 0 ? (
                                    ...
                                    users.map((user) => (
                                        <tr key={user.code} className="hover:bg-gray-50 transition duration-200">
                                            <td className="px-6 py-4 font-mono text-lg text-violet-700 tracking-wider font-semibold">
                                                {user.code}
                                            </td>
                                                ...
                                                {editingCode === user.code ? (
                                                    <div className="flex items-center gap-2">
                                                        <input
                                                            ...
                                                            className="bg-white border border-violet-300 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-violet-100 w-full text-slate-900"
                                                            autoFocus
                                                        />
                                                    </div>
                                                ) : (
                                            ...
                                            <td className="px-6 py-4">
                                                <span className={`px-3 py-1 rounded-full text-xs font-bold border ${user.serviceCount > 0 ? 'bg-violet-50 text-violet-700 border-violet-200' : 'bg-slate-100 text-slate-500 border-slate-200'}`}>
                                            ...
                                            <td className="px-6 py-4 text-right flex items-center justify-end gap-2">
                                                {editingCode === user.code ? (
                                                    <>
                                                        <button
                                                            onClick={saveEditing}
                                                            className="text-violet-700 hover:text-violet-800 p-2 hover:bg-violet-50 rounded-lg transition"
                                                            title="Save"
                                                        >
                                                            <Save size={18} />
                                                        </button>
                                                    ...
                                                    <>
                                                        <button
                                                            ...
                                                            className="text-violet-500 hover:text-violet-700 p-2 hover:bg-violet-50 rounded-lg transition"
                                                            title="Edit Name"
                                                        >
                                                            <Edit2 size={18} />
                                                        </button>
